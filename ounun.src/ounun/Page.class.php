...
class Page
{
    ...
    public function init($page = null,$count = 'count(*)', $config = array())
    {
        ...
        foreach ($returns['np'] as &$v)
        {
            if($v['Begin'])
            {
                $v = $default[0] . '<a href="' . $this->_setUrl($v['Begin']) . '" title="转到第一页">'.$tag[0].'</a>' . $default[1];
            }
            elseif($v['Previous'])
            {
            ...
            elseif($v['Next'])
            {
                $v = $default[0] . '<a href="' . $this->_setUrl($v['Next']) . '" title="下一页">'.$tag[2].'</a>' . $default[1];
            }
            elseif($v['End'])
            {
                $v = $default[0] . '<a href="' . $this->_setUrl($v['End']) . '" title="转到最后一页">'.$tag[3].'</a>' . $default[1];
            }
            else
            {
                $tmp = $v;
                ...
                foreach ($tmp as $value)
                {
                    ...
                    {
                        $v .= $now[0] . '<a href="' . $this->_setUrl($value) . '" title="本页" '.$now[2].' onclick="return false">' . $value . '</a>' . $now[1];
                    }
                    else
                    {
    ...
    public function getTotal($count)
    {
        ...
        return $this->total = (int)$this->_getTotalDb($count);
    }
     ...
     * @return int
     */
    private function _getTotalDb($count)
    {
    	if($this->db->mongo)
    	{
