...
class RennClientBase
{
	...
	function __construct($clientId, $clientSecret)
    {
		...
		$this->tokenStore               = new TokenStoreCookie ();
	}
	...
	public function authWithAuthorizationCode($code, $redirectUri)
    {
		...
		} catch ( ExceptionRenn $e )
        {
			throw new ExceptionClientInvalideAuthorization ( "Authorization failed with Authorization Code. " . $e->getMessage () );
		}
	}
	...
	public function authWithClientCredentials()
    {
        ...
        {
			throw new ExceptionClientInvalideAuthorization ( "Authorization failed with Client Credentials. " . $e->getMessage () );
		}
	}
	...
	public function authWithResourceOwnerPassword($username, $password)
    {
        ...
        {
			throw new ExceptionClientInvalideAuthorization ( "Authorization failed with Resource Owner Password. " . $e->getMessage () );
		}
	}
	...
	public function authWithStoredToken()
    {
		...
		if (! isset ( $token ))
        {
			throw new ExceptionClientInvalideAuthorization ( "Authorization failed with Stored Token. token: null" );
		}
	}
	...
	public  function getTokenFromTokenEndpoint($grantType, $keys, $tokenType = TokenType::MAC)
    {
        ...
        {
			throw new ExceptionRennClient ( "wrong auth type" );
		}
		...
		if (is_array ( $token ) && ! isset ( $token ['error'] )) {
			...
			throw new ExceptionRennClient ( "Get access token failed. " . $token ['error'] . ": " . $token ['error_description'] );
		}
	...
	public function execute($path, $httpMethod, $queryParams, $bodyParams = null, $fileParams = null) {
        ...
        {
			throw new ExceptionRennServer ( $result ['error'] ['code'], $result ['error'] ['message'] );
		}
	...
	private function getAuthorizationHeader($schema, $pathAndQuery, $method)
    {
		...
		if (empty ( $signMethod ))
        {
			throw new ExceptionRennClient ( "wrong mac algorithm" );
		}
