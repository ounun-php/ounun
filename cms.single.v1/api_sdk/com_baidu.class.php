...
class com_baidu
{
    ...
    protected function _do_push($type,$is_today)
    {
        ...
        if($max)
        {
            $cc   = $this->_db->row("SELECT COUNT(`url_id`) as `cc` FROM {$this->_db_sitemap_push} WHERE `Ymd` = :Ymd and `target_id` = :target_id ;",['Ymd'=>$Ymd,'target_id'=>$type]);
            $cc   = (int)$cc['cc'];
            // echo $db->sql()."\n";
            ...
            if($cc < $max)
            {
                ...
                if($is_today)
                {
                    ...
                    $rs          = $this->_db->data_array("SELECT * FROM {$this->_db_sitemap} WHERE {$where_xzh} `lastmod` > :lastmod and `url_id` NOT in ( SELECT `url_id` FROM {$this->_db_sitemap_push} WHERE `Ymd` >= :Ymd and `target_id` = :target_id ) ORDER BY `lastmod` DESC LIMIT 0,".$step_cc.";",$bind);
                }else
                {
                    $bind        = ['Ymd'=>$Ymd_start,'target_id'=>$type];
                    $rs          = $this->_db->data_array("SELECT * FROM {$this->_db_sitemap} WHERE {$where_xzh} `url_id` NOT in ( SELECT `url_id` FROM {$this->_db_sitemap_push} WHERE `Ymd` >= :Ymd and `target_id` = :target_id ) ORDER BY `lastmod` DESC LIMIT 0,".$step_cc.";",$bind);
                }
                if($rs)
                {
    ...
    public function db_sitemap(array $bind, bool $is_update = false)
    {
        ...
        {
            $this->_db->insert($this->_db_sitemap, $bind,'        ',' ON DUPLICATE KEY UPDATE `lastmod`='.((int)$bind['lastmod']).' ');
        }else
        {
            $this->_db->insert($this->_db_sitemap, $bind,' IGNORE ');
        }
    }
    ...
    public function db_sitemap_push(array $urls, int $type,int $Ymd = 0)
    {
        ...
        $this->_db->conn("INSERT INTO {$this->_db_sitemap_push} (`url_id`, `Ymd`, `target_id`) SELECT `url_id`,{$Ymd},{$type} FROM {$this->_db_sitemap} WHERE `loc` in ( ? );",$urls);
    }
    ...
    public function maps_index($url_root='sitemap/list/',string $host='')
    {
        ...
        $total      = $this->_db->row("SELECT count(`url_id`) as `cc` FROM {$this->_db_sitemap};");
        $total      = (int)$total['cc'];
