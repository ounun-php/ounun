...
class base_system_update extends base
{
    /**
     * 执行任务
     * @param array $paras
    ...
    public function data(array $exts,string $site_tag)
    {
        ...
        $url_root           = IsDebug?"http://adm2.moko8.com/api/export/":"https://adm.moko8.com/api/export/";
        $this->_logs_state  = \ounun\logs::state_ok;
        if('libs_v1' == $exts[0])
        {
    ...
    {
        $rs = $this->_db->data_array("SELECT  `tag_id` , COUNT(`data_id`) AS  `count` FROM  {$table_tag_data} where `mod_id` = {$mod_id} GROUP BY  `tag_id` ");
        if($rs)
        {
            foreach($rs as $v)
            {
                ...
                $this->_db->insert_update($table_tag_stat,['mod_id'=>$mod_id,'tag_id'=>$v['tag_id']],$bind);
                // echo $this->_db->sql()."\n";
