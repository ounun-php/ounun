...
class base extends \task\base
{
    ...
    protected $_mode     = \task\manage::mode_dateup;
    /** @var \ounun\mysqli   */
    protected $_db_libs;
    /** @var int 图片wget max */
    ...
    protected function _wget_pics_base(int $pic_id, array $data, array $pic_ext)
    {
        ...
        $this->_db_libs->update($this->_db_table,$bind,' `pic_id` =:pic_id ',['pic_id'=>$pic_id]);
    ...
    {
        $time      = time();
        $tags3     = \api_sdk\com_showapi::tag($pic_title);
        if($tags)
        {
    ...
    {
        $tags3     = \api_sdk\com_showapi::tag($title);
        if($tags)
        {
            $tags3 = array_merge($tags,$tags3);
        }
    ...
    protected  function _insert(array $data, string $fields_name = 'pic_id')
    {
        ...
        if(!$rs)
        {
            $this->_db_libs->insert($this->_db_table,$data);
            echo $this->_db->sql()."\n";
            ...
            {
               $this->msg("sql:".$this->_db_libs->sql() );
    ...
    {
        $rs =  $this->_db_libs->row("SELECT `{$fields_name}` FROM {$this->_db_table} where `{$fields_name}` = :{$fields_name} limit 0,1;",[$fields_name=>$pic_id]);
    ...
    {
        $rs =  $this->_db_libs->row("SELECT `{$fields_name}` FROM {$this->_db_table} ORDER BY `{$fields_name}` DESC limit 0,1;");
        if($rs && $rs[$fields_name])
        {
