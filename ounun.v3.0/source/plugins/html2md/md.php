...
//        }
    ...
    public function md2mip($doc)
    {
        ...
        //img
        $doc = preg_replace('#!\[([^\]]*?)\]\(([^\s]*?)(?: "([^"]*?)")?\)#is', '<mip-img src="\2" alt="\1" title="\3"></mip-img>', $doc);
        // $doc = preg_replace('#!\[\]\(([^\s]*?)\)#is', '<img src="\1" />', $doc);
...
//                  $thead = preg_replace('/<thead[^>]*>(.*)<\/thead>/is',"$1", $table);
//                  $tbody = preg_replace('/<tbody[^>]*>(.*)<\/tbody>/is',"$1", $table);
//                  $thead2 = ($thead == $m[1]) ? '' :$thead;
//                  $tbody2 = ($tbody == $m[1]) ? '' :$tbody;
