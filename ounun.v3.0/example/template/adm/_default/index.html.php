...
<head>
	...
	<style type="text/css">
		...
		.logo_bg:hover{ background-image:url("{$static}{$site_logo_dir}logo_hover.png"); }
	</style>
</head>
				...
				<div class="nav">
					<ul id="topmenu">
						<?php foreach ($purview as $key1 => $data1){ ?>
							<li><em><a href="<?php echo $data1['default']?>" id="header_<?php echo $key1?>" hidefocus="true"  onClick="toggleMenu('<?php echo $key1?>', '<?php echo $data1['default']?>');doane(event);"><?php echo $data1['name']?></a></em></li>
						<?php } ?>
					</ul>
                    ...
                    <div class="sitemapbtn">
                        <a href="#" id="cpmap" onClick="showMap();return false;"><img src="{$static_g}adm/img/btn_map.gif" title="管理中心导航(ESC键)" width="46" height="18" /></a>
                    </div>
				</div>
			</div>
        </td>
				...
				{ ?>
					<ul id="menu_<?php echo $key1?>" style="display: none">
						...
						{ ?>
							...
							if($data2['url'])
							{?>
								<li><a href="<?php echo $data2['url']?>" hidefocus="true" target="main"><em onClick="menuNewwin(this)" title="<?php echo $data2['name']?>"></em><?php echo $data2['name']?></a></li>
								<?php
							}else
							{
								$purview_uuid++;
									...
									<div class="lsub desc" subid="Masb<?php echo $purview_uuid?>">
										...
										<ol style="display:" id="Masb<?php echo $purview_uuid?>">
											...
											foreach ($data2['data'] as $key3 => $data3)
											{
												...
												{
													?>
													<li><a href="<?php echo $data3['url']?>" hidefocus="true" target="main"><em onClick="menuNewwin(this)" title="<?php echo $data3['name']?>"></em><?php echo $data3['name']?></a></li>
													<?php
												}
											} ?>
										</ol>
...
<div class="copyright">
	...
	<p>&copy;2010-<?php echo date('Y')?> <a href="{$powered_corp_url}" target="_blank">{$powered_corp_name_mini}</a> Inc.</p>
</div>
