...
class console
{
    ...
    {
        // print_r(['$argv'=>$argv]);
        if(empty($argv) || empty($argv[1]) || '--help' == $argv[1]) {
            ...
            if($command){
                ...
                static::echo("-- runing... {$run_cmd}   begin:".date("Y-m-d H:i:s")."             --------------------");
                $command->execute($argv);
                $run_time += microtime(true);
                static::echo("-- done      {$run_cmd}     end:".date("Y-m-d H:i:s")." run:".str_pad(round($run_time,4).'s', 8)."--------------------");
            }else{
