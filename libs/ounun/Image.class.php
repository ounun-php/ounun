...
//    }
    ...
     */
    public function thumb($name, $width, $height, $outFileName,$force=false)
    {
        /* 获取图片宽度、高度、及类型信息 */
        ...
        /* 获取新图片尺寸 */
        $size    = $this->getNewSize($width, $height,$imgInfo,$force);
        /* 获取新的图片资源 */
        $newImg  = $this->kidOfImage($srcImg, $size, $imgInfo);
        /* 通过本类的私有方法，保存缩略图并返回新缩略图的名称，以"th_"为前缀 */
        return $this->createNewImage($newImg, $outFileName,$imgInfo);
    }
    ...
     */
    public function waterMark($groundName, $waterName, $waterPos=0, $outFileName)
    {
        /*获取水印图片是当前路径，还是指定了路径*/
...
//        }
        ...
        if(file_exists($groundName) && file_exists($waterName))
        {
            ...
            return $this->createNewImage($groundImg, $outFileName, $groundInfo);
        }else
        {
            echo '图片或水印图片不存在！';
            return false;
    ...
    public function cut($name, $x, $y, $width, $height, $outFileName)
    {
        ...
        return $this->createNewImage($cutimg, $outFileName,$imgInfo);
    }
    ...
    private function getNewSize($width, $height, $imgInfo,$force)
    {
    ...
    private function createNewImage($newImg, $newFileName, $imgInfo)
    {
        //$this->path = rtrim($this->path,"/")."/";
        switch ($imgInfo["type"])
        {
            case 1: //gif
                $result = imageGIF($newImg, $newFileName);
                break;
            case 2: //jpg
                $result = imageJPEG($newImg,$newFileName);
                break;
            case 3: //png
                $result = imagePng($newImg, $newFileName);
                break;
        }
        imagedestroy($newImg);
        return $newFileName;
