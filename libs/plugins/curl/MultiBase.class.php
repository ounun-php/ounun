...
class MultiBase
{
	...
	private function addTaskPool($task)
    {
		...
		{
			foreach (['taskPoolAhead',  'taskPool' ] as $v )
			{
			...
			while ( false != ($curlInfo = curl_multi_info_read ( $this->mh, $this->info ['all'] ['queueNum'] )) ) {
				...
				if ($curlInfo ['result'] !== CURLE_OK || isset ( $this->userError ))
				{
					if ($task [self::TASK_TRYED] >= $this->maxTry)
					{
						...
						{
							$err = ['error' => $this->userError ];
						} else {
							$err = ['error' => [$curlInfo ['result'], curl_error ( $ch ) ]];
						}
						$err ['info'] = $info;
	...
	private function callCbInfo($force = false)
    {
		...
		if (($force || $now - $lastTime > 0) && isset ( $this->cbInfo ))
		{
			...
			call_user_func_array ( $this->cbInfo, [$this->info ]);
		}
	}
		...
		while ( $c > 0 )
        {
			...
			} else
			{
				...
				if (0 < ($this->maxThread - count ( $this->taskPool )) and ! empty ( $this->cbTask ))
				{
					...
					call_user_func_array ( $this->cbTask [0], [$this->cbTask [1]]);
				}
			...
			if (! empty ( $task ))
			{
				...
				if (! $cache)
				{
					...
					if (($isNoType && $this->info ['all'] ['taskRunningNumNoType'] < $maxThread)
                        || (! $isNoType && $this->info ['all'] ['taskRunningNumType'] [$task [self::TASK_ITEM_CTL] ['type']] < $maxThread))
					{
